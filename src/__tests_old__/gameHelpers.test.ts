...
describe("GameHelpers", () => {
  ...
  describe("resetRevealedMap", () => {
    ...
    it("returns a map of booleans", () => {
      ...
      for (
        let index = 0;
        index < GameHelpers.resetRevealedMap().values.length;
        index += 1
      ) {
  ...
  describe("gameFinished", () => {
    it("returns true if turns == 0", () => {
      expect(
        GameHelpers.gameFinished(0, [..."ABCDEFGHIJKLMNOPQRSTUVWXYZ"]),
      ).toBeTruthy();
    ...
    it("returns false otherwise", () => {
      ...
      expect(
        GameHelpers.gameFinished(8, [..."ABCDEFGHIJKLMNOPQRSTUVWXYZ"]),
      ).not.toBeTruthy();
    });
  });
});
